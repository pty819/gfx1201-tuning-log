...
2. **r4d 手写注意力从未生效**：r4d.so 按固定几何编译（`r4d.ATTN_HEAD_DIM/ATTN_GQA/ATTN_BLOCK_SIZE` = 256 / 6 / 16），
   Hy-MT2 是 h128/gqa4/块32——`r4d.select()` 返回 None → **静默回退上游 stock Triton attention（不崩，只打印拒绝原因）**。
...
3. **stock Triton 的 fp8 分支慢**：fp8 KV 走 per-tensor + inline descale 进统一 kernel；SM89+ 硬门只对 `is_cuda()` 生效（ROCm 放行）。
   ...
   铁证：fp8 KV 时 prefill 4647 ≈ bf16 4648 t/s（prefill 读 fp8 KV 毫发无损）→ **病灶只在 decode kernel**。
...
| 4 路 | **147.9 t/s** |
| 8 路 | **223.4 t/s** |
| prefill | 4385 tok/s（vs ROCM_ATTN 4647，-6%） |
| TTFT | 0.90 s |
| KV 池 | 80,960 tokens（9.88× @8k；启动显存干净时比 67.7k 还大） |

并发全面超 bf16 KV 配置（139/210），池子是 bf16 的 2 倍，翻译质量无退化（bench_tr2 md5 对照）。
