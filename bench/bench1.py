...
BASE, MODEL, CONC = sys.argv[1].rstrip('/'), sys.argv[2], int(sys.argv[3]) if len(sys.argv) > 3 else 1
PT, NT, ROUNDS = 4096, 256, 2
PT_USE = PT
S = requests.Session()
...
INSTR = "Count from one to three hundred, one number per line. Do not stop early.\n\n"

def chat(prompt, max_tokens, stream=True):
    body = {"model": MODEL, "messages": [{"role": "user", "content": prompt}],
            "temperature": 0, "max_tokens": max_tokens, "stream": stream, "ignore_eos": True}
...
PT_USE = chat(INSTR + FILL * k, 1, False)[0]
print(f"conc={CONC} calib: prompt_tokens={PT_USE}")
...
def run_round(tag, max_tokens):
    ...
    for t in th: t.join()
    wall = time.time() - t0
    ttfts = [r[2] for r in res]
    if max_tokens == 1:
        pp = [r[0] / r[2] for r in res if r[2] > 0]
        print(f"{tag}: wall={wall:.2f}s ttft={['%.2f'%x for x in ttfts]} pp_stream={['%.0f'%x for x in pp]}")
    else:
        per_tg = [((r[1] - 1) / (r[3] - r[2])) if r[3] > r[2] else 0 for r in res]
        print(f"{tag}: ttft={['%.2f'%x for x in ttfts]} tg_stream={['%.1f'%x for x in per_tg]} agg={sum(per_tg):.1f}")

for r in range(ROUNDS):
    run_round(f"P{r+1}(single-pp)", 1)
for r in range(ROUNDS):
    run_round(f"T{r+1}(single-tg)", NT)
