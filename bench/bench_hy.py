...
def build_prompt(worker, reps):
    paras = PARAS[worker % 4:] + PARAS[:worker % 4]   # unique prefix per worker
    doc = " ".join(p + "\n\n" for p in (paras * 6)[:reps])
...
def nonstream(prompt, max_tokens=1):
    t0 = time.time()
...
REPS = 26   # tune so prompt ~= 4k tokens

print("== calibrate prompt size (worker0, max_tokens=1) ==")
t, pt = nonstream(build_prompt(0, REPS))
print(f"prompt_tokens={pt}  prefill_time={t:.2f}s  -> {pt/t:.0f} tok/s prefill (single)")

print("\n== 4x concurrent prefill-only (max_tokens=1, distinct docs) ==")
res = {}
def w(i):
    res[i] = nonstream(build_prompt(i, REPS))
...
for i in range(4):
    t, pt = res[i]
    print(f"  w{i}: pt={pt} time={t:.2f}s")
print(f"  wall={wall:.2f}s  aggregate prefill ~= {sum(r[1] for r in res.values())/wall:.0f} tok/s")

print("\n== single-stream long convo (stream 512 out) ==")
ttft, total, pt, ct = stream(build_prompt(1, REPS))
print(f"pt={pt} ct={ct}  ttft={ttft:.2f}s total={total:.2f}s  decode ~= {ct/(total-ttft):.1f} tok/s")

print("\n== 4 concurrent long convos (stream 512 out each) ==")
res = {}
def w2(i):
    res[i] = stream(build_prompt(i, REPS))
...
print(f"  wall={wall:.2f}s  per-stream avg={agg/4:.1f} tok/s  aggregate decode={agg:.0f} tok/s")
