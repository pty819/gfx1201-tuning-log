...
PT_USE = ptok
print(f"calib: prompt_tokens={ptok}")
...
def run_round(tag):
    ...
    for t in th: t.join()
    wall = time.time() - t0
    otoks = sum(r[1] for r in res)
    dec = [r[3] - r[2] for r in res]
    tg_agg = otoks / max(dec)
    per_tg = [((r[1] - 1) / (r[3] - r[2])) if r[3] > r[2] else 0 for r in res]
    print(f"{tag}: out={otoks} wall={wall:.2f}s tg_agg={tg_agg:.1f} tg_stream={['%.1f'%x for x in per_tg]} sum_stream={sum(per_tg):.1f}")

for r in range(ROUNDS):
    run_round(f"T{r+1}(ignore_eos, 4x{NT})")
